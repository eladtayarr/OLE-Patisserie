(function () {
            ...
            if (isMobile()) {
                ...
                animatedElements = animatedElements.filter(function (el) {
                    if (!el.parents().filter(function (parentElement) {
                        if (parentElement.matches('a, p, .navbar, .mbr-arrow, footer, .iconbox, .mbr-slider, .mbr-gallery, .mbr-testimonial .card-block, #cookiesdirective, .mbr-wowslider, .accordion, .tab-content, .engine, #scrollToTop, .modal-content')) {
                            return true
                        }
                    }).length) {
                        return true
    ...
    if (!isBuilder) {
        ...
        const dropdownTogglersArray = document.querySelectorAll('.dropdown-toggle');
        const dropdownMenusArray = document.querySelectorAll('.dropdown-menu');
        dropdownTogglersArray.forEach(dropdownTogglerItem => {
            const isDropdownTogglerShow = dropdownTogglerItem.classList.contains('show');
            if (isDropdownTogglerShow) {
